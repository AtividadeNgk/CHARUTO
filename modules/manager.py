...
def create_bot_activity_table():
    """Cria tabela para rastrear atividade dos bots"""
    conn = sqlite3.connect('data.db')
    cur = conn.cursor()
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS BOT_ACTIVITY (
            ...
            last_activity TEXT,
            is_hibernated INTEGER DEFAULT 0,
            hibernated_at TEXT,
...
def update_bot_activity(bot_id):
    ...
    # Insere ou atualiza a atividade
    cursor.execute("""
        INSERT INTO BOT_ACTIVITY (bot_id, last_activity, is_hibernated)
        VALUES (?, ?, 0)
...
def get_bot_activity(bot_id):
    """Retorna informações de atividade do bot"""
    conn = sqlite3.connect('data.db')
    cursor = conn.cursor()
    
    cursor.execute("""
        SELECT last_activity, is_hibernated, hibernated_at
        FROM BOT_ACTIVITY
        WHERE bot_id = ?
    """, (bot_id,))
    ...
    if result:
        return {
            ...
            'is_hibernated': bool(result[1]),
            'hibernated_at': result[2]
        }
    
    # Se não existe registro, cria um novo
    update_bot_activity(bot_id)
    return {
...
def hibernate_bot(bot_id):
    ...
    cursor.execute("""
        UPDATE BOT_ACTIVITY
        SET is_hibernated = 1, hibernated_at = ?
        WHERE bot_id = ?
    """, (now, bot_id))
...
def wake_bot(bot_id):
    """Acorda um bot hibernado"""
    conn = sqlite3.connect('data.db')
    cursor = conn.cursor()
    
    cursor.execute("""
        UPDATE BOT_ACTIVITY
        SET is_hibernated = 0, hibernated_at = NULL
        WHERE bot_id = ?
    """, (bot_id))
...
def get_all_hibernated_bots():
    """Retorna lista de bots hibernados"""
    conn = sqlite3.connect('data.db')
    cursor = conn.cursor()
    
    cursor.execute("""
        SELECT bot_id FROM BOT_ACTIVITY
        WHERE is_hibernated = 1
    """)
...
def calculate_days_inactive(last_activity_str):
    """Calcula quantos dias o bot está inativo"""
    if not last_activity_str:
        return 999  # Sem registro = muito tempo
