...
def print_startup_progress(current, total, prefix=''):
    """Mostra barra de progresso durante inicialização"""
    bar_length = 40
...
# Configurações de inicialização gradual
STARTUP_BATCH_SIZE = int(os.environ.get("STARTUP_BATCH_SIZE", 10))  # Tamanho do lote
STARTUP_BATCH_DELAY = int(os.environ.get("STARTUP_BATCH_DELAY", 5))  # Delay em segundos
...
def initialize_all_registered_bots():
    ...
    for bot in bots:
        ...
        if activity['is_hibernated']:
            print(f"Bot {bot_id} está hibernado - pulando inicialização")
            continue
            
        bots_ativos.append(bot)
    
    print(f'Inicializando {len(bots_ativos)} bots ativos de forma gradual...')
    
    # NOVA IMPLEMENTAÇÃO: Inicialização em lotes
    TAMANHO_LOTE = STARTUP_BATCH_SIZE
    DELAY_ENTRE_LOTES = STARTUP_BATCH_DELAY
    
    # Divide os bots em lotes
    for i in range(0, len(bots_ativos), TAMANHO_LOTE):
        lote = bots_ativos[i:i + TAMANHO_LOTE]
        lote_num = (i // TAMANHO_LOTE) + 1
        total_lotes = ((len(bots_ativos) - 1) // TAMANHO_LOTE) + 1
        
        print(f'\n📦 Iniciando lote {lote_num}/{total_lotes} ({len(lote)} bots)...')
        
        for j, bot in enumerate(lote):
            bot_id = bot[0]
            
            # Mostra progresso do lote atual
...
def start_bot(new_token, bot_id):
    ...
    # Garante que o bot não está marcado como hibernado
...
def monitor_hibernated_bots():
    """Monitora bots que precisam ser acordados"""
    while True:
        try:
            # Pega todos os bots
            ...
            for bot in all_bots:
                ...
                # Se o bot não está hibernado mas não tem processo ativo
                if not activity['is_hibernated'] and bot_id not in processes:
                    print(f"Bot {bot_id} precisa ser iniciado...")
                    try:
                        start_bot(bot[1], bot_id)
                        print(f"Bot {bot_id} iniciado pelo monitor")
                    except Exception as e:
                        print(f"Erro ao iniciar bot {bot_id}: {e}")
                
                # Se o bot não está hibernado mas o processo morreu
                elif not activity['is_hibernated'] and bot_id in processes:
                    if not processes[bot_id].is_alive():
                        print(f"Processo do bot {bot_id} morreu. Reiniciando...")
                        processes.pop(bot_id)
                        try:
                            start_bot(bot[1], bot_id)
                        except Exception as e:
            ...
            print(f"Erro no monitor de hibernação: {e}")
            time.sleep(30)

def start_hibernation_monitor():
    monitor = Process(target=monitor_hibernated_bots)
    monitor.start()
    return monitor
...
def get_system_resources():
    """Retorna informações de CPU e RAM do sistema"""
    if session.get("auth", False):
        try:
            # CPU
            ...
            # Processo Python (app.py)
            process = psutil.Process()
            process_memory = process.memory_info().rss / (1024 * 1024)  # MB
            
            # Conta processos dos bots
            bot_processes = len(processes)
            
            # Determina status baseado no uso
...
def get_active_bots():
    if session.get("auth", False):
        ...
        for bot in all_bots:
            ...
            # Pega informações de atividade
            activity = manager.get_bot_activity(bot_id)
            days_inactive = manager.calculate_days_inactive(activity['last_activity'])
            ...
            bot_info = {
                ...
                'is_hibernated': activity['is_hibernated'],
                'last_activity': activity['last_activity']
            }
            
            # Verifica se o bot está hibernado
            if activity['is_hibernated']:
                bot_info['status'] = 'hibernated'
            # Verifica se o processo está ativo
            elif bot_id in processes:
...
@app.route('/api/bot/hibernate/<bot_id>', methods=['POST'])
def hibernate_bot(bot_id):
    if session.get("auth", False):
        bot = manager.get_bot_by_id(bot_id)
        if bot:
            try:
                ...
                if str(bot_id) in processes:
                    ...
                    processes.pop(str(bot_id))
                    print(f"Processo do bot {bot_id} parado para hibernação")
                
                # Remove dos dados em memória
                if str(bot_id) in bots_data:
                    bots_data.pop(str(bot_id))
                
                # Marca como hibernado no banco
                manager.hibernate_bot(bot_id)
                
                return jsonify({
                    "success": True,
                    "message": f"Bot {bot_id} hibernado com sucesso!"
                })
                
            except Exception as e:
                return jsonify({
                    "success": False,
                    "message": f"Erro ao hibernar bot: {str(e)}"
                }), 500
        
        return jsonify({"error": "Bot não encontrado"}), 404
    return jsonify({"error": "Unauthorized"}), 403

@app.route('/api/bot/wake/<bot_id>', methods=['POST'])
def wake_bot(bot_id):
    if session.get("auth", False):
        bot = manager.get_bot_by_id(bot_id)
        if bot:
            try:
                # Acorda o bot no banco
                manager.wake_bot(bot_id)
                
                # Reinicia o bot
                if str(bot_id) not in processes or not processes[str(bot_id)].is_alive():
                    start_bot(bot[1], bot_id)
                    
                    return jsonify({
                        "success": True,
                        "message": f"Bot {bot_id} acordado com sucesso!"
                    })
                else:
                    return jsonify({
                        "success": True,
                        "message": f"Bot {bot_id} já está ativo!"
                    })
                    
            except Exception as e:
                return jsonify({
                    "success": False,
                    "message": f"Erro ao acordar bot: {str(e)}"
                }), 500
...
if __name__ == '__main__':
    ...
    manager.create_bot_activity_table()  # NOVA LINHA ADICIONADA
    initialize_all_registered_bots()
    start_register()
    start_hibernation_monitor()  # NOVA LINHA ADICIONADA
