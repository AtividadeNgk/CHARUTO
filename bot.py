...
def track_activity(bot_id):
    """Rastreia atividade do bot"""
    try:
        manager.update_bot_activity(bot_id)
    except Exception as e:
        print(f"Erro ao rastrear atividade: {e}")
        
async def check_and_wake_bot(bot_id, context):
    """Verifica se o bot está hibernado e acorda se necessário"""
    try:
        activity = manager.get_bot_activity(bot_id)
        if activity['is_hibernated']:
            print(f"Bot {bot_id} está hibernado. Marcando para acordar...")
            
            # Apenas marca o bot para acordar (remove hibernação)
            manager.wake_bot(bot_id)
            
            # NÃO tenta enviar mensagem porque o bot está parado
            # O monitor_hibernated_bots irá detectar e reiniciar o bot
            
            return True  # Retorna True para indicar que estava hibernado
    except Exception as e:
        print(f"Erro ao verificar hibernação: {e}")
...
async def run_bot(token, bot_id):
    ...
    print(f"[DEBUG] Iniciando bot {bot_id}")
    
    # Verifica se o bot está hibernado
    activity = manager.get_bot_activity(bot_id)
    print(f"[DEBUG] Status de hibernação: {activity['is_hibernated']}")
    
    if activity['is_hibernated']:
        print(f"Bot {bot_id} está hibernado. Não será iniciado.")
        return
    
    disable_get_updates(token)
    bot_application = Application.builder().token(token).build()
    
    # Wrapper para rastrear atividade em TODOS os handlers
    def activity_wrapper(handler):
        async def wrapped(update, context):
            # Primeiro verifica se precisa acordar
            if await check_and_wake_bot(bot_id, context):
                # Se acordou, não processa o comando atual
                return
            
            track_activity(bot_id)
            return await handler(update, context)
        return wrapped
    
    # IMPORTANTE: CommandHandlers devem vir ANTES do MessageHandler genérico
    # Handlers de comandos específicos com tracking
    bot_application.add_handler(CommandHandler('start', activity_wrapper(start)))
    bot_application.add_handler(CommandHandler('comandos', activity_wrapper(comandos)))
    
    # ConversationHandlers - vamos adicionar tracking em cada um
    # Modificamos cada entry_point para incluir tracking
    for handler in [conv_handler_grupo, conv_handler_upsell, conv_handler_planos, 
                    conv_handler_adeus, conv_handler_recuperacao, conv_handler_inicio,
                    conv_handler_admin, conv_handler_gateway, conv_handler_disparo,
                    conv_handler_downsell, conv_handler_orderbump]:
        # Adiciona tracking no entry point
        original_entry = handler.entry_points[0]
        wrapped_entry = CommandHandler(
            original_entry.commands[0],
            activity_wrapper(original_entry.callback)
        )
        handler.entry_points[0] = wrapped_entry
        bot_application.add_handler(handler)
    
    # Handlers de chat
    bot_application.add_handler(ChatJoinRequestHandler(activity_wrapper(check_join_request)))
    
    # CallbackQueryHandlers com tracking
    bot_application.add_handler(CallbackQueryHandler(activity_wrapper(pagar), pattern='^pagar_'))
    bot_application.add_handler(CallbackQueryHandler(activity_wrapper(acessar_planos), pattern='^acessar_ofertas$'))
    bot_application.add_handler(CallbackQueryHandler(activity_wrapper(confirmar_plano), pattern='^plano_'))
    bot_application.add_handler(CallbackQueryHandler(activity_wrapper(processar_upsell), pattern='^upsell_'))
    bot_application.add_handler(CallbackQueryHandler(activity_wrapper(processar_downsell), pattern='^downsell_'))
    bot_application.add_handler(CallbackQueryHandler(activity_wrapper(exibir_plano), pattern='^exibir_'))
    bot_application.add_handler(CallbackQueryHandler(activity_wrapper(processar_orderbump), pattern='^orderbump_'))
    bot_application.add_handler(CallbackQueryHandler(activity_wrapper(cancel), pattern='cancelar'))
    
    # IMPORTANTE: MessageHandler genérico deve ser o ÚLTIMO
    bot_application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, activity_wrapper(acessar_planos_force)))
    
    bot_application.bot_data['id'] = bot_id
    
    print(f"[DEBUG] Bot {bot_id} - Inicializando aplicação...")
    await bot_application.initialize()
    await bot_application.start()
    await bot_application.updater.start_polling()
    print(f"[DEBUG] Bot {bot_id} - Polling iniciado com sucesso!")
